...
func (h *Handler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/v1/master/rooms", h.List)
	mux.HandleFunc("POST /api/v1/master/rooms", h.Create)
	mux.HandleFunc("GET /api/v1/master/rooms/{id}", h.GetByID)
	mux.HandleFunc("PUT /api/v1/master/rooms/{id}", h.Update)
	mux.HandleFunc("DELETE /api/v1/master/rooms/{id}", h.Delete)
}
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	item, err := buildRoom(req.Code, req.Name, req.Type, req.Capacity)
	if err != nil {
	...
	if err != nil {
		...
		case errors.Is(err, ErrDuplicateCode):
			response.Error(w, http.StatusConflict, "room code already exists")
		case errors.Is(err, ErrDuplicateName):
			response.Error(w, http.StatusConflict, "room name already exists")
		default:
			response.Error(w, http.StatusInternalServerError, err.Error())
		}
		return
...
func buildRoom(code, name, roomType string, capacity *int) (Room, error) {
	code = strings.ToUpper(strings.TrimSpace(code))
	if code == "" {
		return Room{}, fmt.Errorf("code is required")
	}

	name = strings.TrimSpace(name)
	if name == "" {
		return Room{}, fmt.Errorf("name is required")
	}

	roomType = strings.TrimSpace(roomType)
	if capacity != nil && *capacity < 0 {
		return Room{}, fmt.Errorf("capacity must be greater than or equal to 0")
	}
